...
def assess_guardrail(
    ...
    for index, guardrail in enumerate(guardrails):
        ...
        if output=="unsafe":
            guardrails_report.append({"Risk": high_and_transition_risks[index], "old_msg": state.prompt, "new_msg": "Output cannot be provided in this context"})
...
class GuardrailsAgent(Agent):
    ...
    def _build_graph(
        ...
        # Add nodes
        graph.add_node("Assess Guardrails", partial(assess_guardrail, inference_engine, taxonomy))
